...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('duobao-upms-systemid', systemid);
		$.cookie('duobao-upms-systemname', systemname);
		$.cookie('duobao-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('duobao-upms-systemid') || 1;
	var systemname = $.cookie('duobao-upms-systemname') || 'duobao-upms-server';
	var systemtitle = $.cookie('duobao-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
